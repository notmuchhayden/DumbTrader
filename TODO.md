# 앞으로 해야 할 일들

## Login 개발
- [x] 로그인 화면 UI 구현
- [x] 로그인 정보 저장/불러오기 기능 리팩토링
- [x] 기존에 Login 과 Account 정보가 섞여있던 부분을 분리

## 계좌 관리 시스템 개발
- [x] 계좌번호 불러오기 테스트
	- [x] GetAccountListCount() 함수 테스트
	- [x] GetAccountList() 함수 테스트
	- [x] GetAccountName() 함수 테스트
- [x] 계좌 번호를 통해 계좌 정보 불러오기
- [x] AccountInfo 클래스 구현
- [x] 계좌 정보를 Entity Framework 모델로 변환
- [ ] 조회후 데이터를 데이터베이스에 저장 <== 여기 작업중
- [ ] 계좌 잔고 조회 기능 구현
- [ ] 계좌 정보 출력 기능 구현

## 화면 layout 개발
- [x] 메인 화면 레이아웃 설계 및 구현
- [x] 계좌관리 화면 레이아웃 설계 및 구현

### 관심 종목 관리화면 개발 <== 여기 작업중
- [ ] 주식 종목 리스트 화면 - t8430
	- [x] UI 기획
	- [x] WatchlistView, WatchlistViewModel 구현
	- [ ] UI 기획 이미지로 부터 실제 UI 구현
- [ ] SidebarView 에서 '관심종목관리' 버튼 클릭시 관심 종목 관리화면으로 이동 기능 구현
- [ ] 조회 버튼 추가. 버튼 클릭시 주식 t8430 을 통해서 주식 종목 리스트 조회
- [ ] 주식 종목 리스트를 조회가 완료되면 데이터베이스에 저장
- [ ] 종목 리스트 검색 기능 추가
- [ ] 관심 종목 추가/삭제 기능 추가. 관심 종목은 종목 리스트의 왼쪽에 종목 이름만 나열

### 개별 종목 관리 화면 개발
- [ ] 종목코드를 넣으면 현재 주가를 조회하는 화면 개발 - t1101
- [ ] 상세한 디자인은 증권사 HTS 를 참고
- [ ] 추후 종목 리스트에서 더블클릭시 개별 종목 정보 화면으로 이동하는 기능 추가
- [ ] 매매 전략 화면은 개별 종목 관리 화면에 통합
#### 매매 전략 화면 개발
- [ ] 왼쪽에 관심 종목 리스트가 나오고, 우측에 매매 전략을 설정하는 화면 개발
- [ ] 매매 전략은 Roslyn 을 이용해서 C# 코드를 작성할 수 있도록 구현

### 대시보드 화면 개발
- [ ] 대시보드 화면의 이름을 다음주 하나로 변경
	- Watchlist (관심종목)
	- Auto-Trading Monitor (자동매매 모니터)
	- Portfolio Overview (포트폴리오 요약)
	- Trading Summary (매매 요약)
	- Market Watch (마켓 워치)
	- Strategy Board (전략 보드)
	- Stock Monitor (종목 모니터)
	- Favorites Panel (관심종목 패널)
- [ ] XARealService 에서 실시간 데이터를 수신하면 모델을 변경하고 자동으로 대시보드가 갱신되도록 구현

## 조회 TR 개발
- [x] XAQuery 클래스 구현
- [ ] 현재 주가 조회 테스트

## 실시간 TR 개발
- [x] XAReal 클래스 구현
- [ ] 실시간 시세 수신 테스트


## 1차 배포
- [ ] 2026년 1월 17일
